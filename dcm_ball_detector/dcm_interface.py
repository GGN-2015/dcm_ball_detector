...
def get_log_numpy_array_from_dcm_file(filepath: str):
    assert os.path.isfile(filepath)
    return np.log(get_raw_numpy_array_from_dcm_file(filepath)[0] + 1)
...
# 对文件夹中正则化后的数据获得极差图样
@functools.cache
def get_range_matrix_for_log_znorm_in_folder(folder: str):
    assert os.path.isdir(folder)
...
def show_range_matrix_for_folder(folder: str, thresh: float):
    assert 0 < thresh < 1
    range_matrix = get_range_matrix_for_log_znorm_in_folder(folder)
...
# 不要在生产环境中使用此功能
def show_debug_all_file_in_folder_as_bin(folder: str):
    assert os.path.isdir(folder)
    min_val, max_val = get_min_max_value_of_log_dataset_folder(folder)
    for file in os_interface.dir_file_scan(folder, ".dcm"):
        np_array = get_log_znorm_numpy_array_from_dcm_file(file, min_val, max_val)
        show_debug_numpy_array(np_array)
