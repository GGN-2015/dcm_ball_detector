...
# 给定中心点坐标
# 获取采样坐标盒子
def get_box_range(tnow: int, xnow: int, ynow: int, max_x_range, max_y_range, max_t_range):
    ...
    ymin = ynow - Y_RADIUS
    ymax = ynow + Y_RADIUS
    tmin, tmax = range_unify(tmin, tmax, 0, max_t_range)
...
def get_cube_from_log_numpy_list_in_folder_around_center(folder:str, tnow:int, xnow:int, ynow:int):
    max_x_range, max_y_range, max_t_range = dcm_interface.get_max_xyt_range_from_folder(folder)
    tmin, tmax,xmin, xmax, ymin, ymax = get_box_range(tnow, xnow, ynow, max_x_range, max_y_range, max_t_range)
    ...
    for i in range(len(index_list)):
        ...
        if len(index_to_coord_set_map[index]) > 0: # 说明能够找到至少一个类似物
            for (center_x, center_y) in index_to_coord_set_map[index]:
                box_rng = get_box_range(index, center_x, center_y, max_x_range, max_y_range, max_t_range)
